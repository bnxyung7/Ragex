...
- Import from Files by opening or sharing a `.3105` package to X.
...
- On a new device or installation, a protected package asks for its password once. X stores the unlocked content key in Keychain; the exported package remains encrypted and tied to its original password.
