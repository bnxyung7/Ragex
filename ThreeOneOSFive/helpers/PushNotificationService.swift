    ...
    private func uploadTokenToServer(token: String) {
        ...
        Task {
            do {
                ...
                // Check if it's a 405 error (server doesn't support push)
                let isUnsupported = (error as? KeyAPIError).map { err in
                    if case .httpError(let code) = err, code == 405 {
                        return true
                    }
                    return false
                } ?? false
                ...
                await MainActor.run {
                    if isUnsupported {
                        // Don't queue for retry if server explicitly doesn't support push
                        print("[Push] ⚠️ Server doesn't support push notifications — skipping registration")
                        self.uploadedSignature = signature  // Mark as "completed" to avoid retries
                        self.clearPendingUpload()
                    } else {
                        // Network or other temporary error — queue for retry
