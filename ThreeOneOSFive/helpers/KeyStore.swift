...
class KeyStore: ObservableObject {
    ...
    private func validateAndActivateRemotely(keyString: String, deviceId: String, completion: @escaping (Result<UserSession, KeyActivationError>) -> Void) async {
        do {
            ...
            await MainActor.run {
                // Check if update is genuinely required
                ...
                if let updateRequired = validationResult.updateRequired, updateRequired == true,
                   ...
                   AppUpdateChecker.isNewer(minVersion, than: currentVersion) {
                    print("[KeyStore] ⚠️ Update required: Key requires version \(minVersion), current is \(currentVersion)")
                ...
                if validationResult.valid {
                        ...
                        Task {
                            do {
                                ...
                                await MainActor.run {
                                    ...
                                    if let apiErr = error as? KeyAPIError {
                                        switch apiErr {
                                        case .serverMessage(let msg):
                                            if msg.lowercased().contains("ya está activada") || msg.lowercased().contains("already") {
                                                // Create session if it was already activated
                                                self.createSession(keyString: keyString, completion: completion)
                                                return
                                            }
                                            completion(.failure(.serverError(message: msg)))
                                        case .httpError(let code):
                                            completion(.failure(.serverError(message: "Error de servidor (\(code))")))
                                        default:
                    ...
                    if isBanned {
                        ...
                        } else if reason.contains("another device") {
                            completion(.failure(.alreadyActivated))
                        } else {
                            completion(.failure(.serverError(message: reason)))
                        }
                    } else {
            ...
            await MainActor.run {
                ...
                if let apiErr = error as? KeyAPIError, case .serverMessage(let msg) = apiErr {
                    completion(.failure(.serverError(message: msg)))
                } else {
                    completion(.failure(.serverError(message: "Error de conexión con el servidor. Intenta de nuevo.")))
                }
            }
        }
    }
